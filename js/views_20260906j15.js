...
(function (global) {
  ...
  function viewPolitics() {
    ...
    const kaodianHTML = kaodianOfDay.map((k, i) =>
      '<details class="custom">' +
        '<summary>📌 ' + U.esc(k.title) +
    ...
    return moduleShell('politics', '政治理论', '🏛️', [
      ...
      '</div>' +
      '<div class="card">' +
        '<div class="card-title"><span class="emoji">💎</span>核心考点<span class="card-sub">（每日' + kaodianOfDay.length + '条 · 点击展开要点）</span></div>' +
    ...
    const kjCard = kjOfDay ? (
      (function() {
        ...
        return '<div class="card kj-card">' +
          '<div class="card-title"><span class="emoji">📜</span>口诀速背<span class="card-sub">（常识88条 · 每日1条 · 共' + kjList.length + '条）</span>' + progHTML + '</div>' +
  ...
  function idiomStatCard() {
    ...
    return '<div class="card">' +
      ...
      '<div class="im-stat">' +
        ...
        '<div class="im-box"><div class="im-num">' + st.reviewMean + '</div><div class="im-lab">待复习(条)</div></div>' +
      '</div>' +
    '</div>';
  }
  function viewLanguage() {
    ...
    return moduleShell('language', '言语理解', '💬', [
      ...
      '<div class="card">' +
        '<div class="card-title"><span class="emoji">📖</span>今日成语辨析<span class="card-sub">（每日1组 · 库共' + C.IDIOMS.length + '对 · 覆盖国考/福建省考高频）</span></div>' +
        '<div class="section-tip">精选国考与福建省考高频近义易混成语，点击展开查看用法区别及近5年真题出现频次（国考/福建）。</div>' +
        pairs.map(p =>
          '<details class="custom"><summary>🔤 ' + U.esc(p.a) + ' ↔ ' + U.esc(p.b) +
  ...
  function viewStats() {
    ...
    for (const key in mods) {
      ...
      // 题库覆盖 = 累计练习数 / 题库总量 → 比例 + 颜色
      const total = _modTotal(key);
      let coverHTML = '';
      if (total > 0) {
        // cnt 是"已练过次数",理论上不会超过 total*倍数,但为防溢出截断
    ...
    return '' +
      ...
      '<div class="card">' +
        '<div class="card-title"><span class="emoji">📊</span>各模块做题量（柱状图）</div>' +
        '<div class="chart-box" data-chart="bar" data-chartdata="' + U.esc(chartData) + '"></div>' +
      '</div>' +

      '<div class="card">' +
        '<div class="card-title"><span class="emoji">🥧</span>各模块正确率（环形图）</div>' +
        '<div class="chart-box" data-chart="pie" data-chartdata="' + U.esc(chartData) + '"></div>' +
      '</div>' +
      ...
      // ④ 每日错题趋势（近14天折线）
      '<div class="card">' +
        '<div class="card-title"><span class="emoji">📉</span>每日新增错题（近14天）</div>' +
        '<div class="chart-box" data-chart="line" data-chartdata="' + U.esc(misTrendData) + '"></div>' +
        '<div class="section-tip">观察错题是否随复习巩固逐步收敛。今日新增 <b style="color:#d9534f">' + misToday + '</b> 道。</div>' +
      '</div>' +
  ...
  function bindDailyQuiz(module, pool, opts) {
    ...
    // 把新增答对/总累加到 stats（每答一题或刷新页面后调用，按增量累加避免重复/漏记）
    function flushStats() {
      const answeredCount = records.filter(r => r !== null).length;
      if (answeredCount <= lastLoggedCount) return;
      const added = answeredCount - lastLoggedCount;
      // 统计本批新增中答对数
      let newCorrect = 0;
      for (let i = lastLoggedCount; i < answeredCount; i++) {
        const r = records[i];
        if (r && r.correct) newCorrect++;
      }
      // 时长按"本题推荐时限"折算（最小 1 分钟，避免 0 显示）
      const addedMin = Math.max(1, Math.round(added * perQLimit / 60));
      try {
        if (typeof U.logStudy === 'function') {
          U.logStudy(module, added, addedMin, newCorrect, added);
        }
      } catch (err) {}
      lastLoggedCount = answeredCount;
      try {
        const lk = Store.PREFIX + 'dq_logged_' + module + '_' + U.todayStr();
      ...
      if (!isAnswered) {
        area.querySelectorAll('.option').forEach(op => {
          op.addEventListener('click', function (e) {
            ...
            records[cur] = { choice: i, correct: correct, time: spent };
            persist();  // 保存进度，刷新后不丢失
            render();  // 重新渲染显示结果
            if (correct) {
              U.toast('🎉 回答正确！');
  ...
  function viewMistake() {
    ...
    weeks.forEach(week => {
      ...
      blocks +=
        '<details class="custom wsum-card">' +
          '<summary>' +
  ...
  function weekWeakness(byModule) {
    let maxMod = null, maxN = 0;
    for (const k in byModule) { if (byModule[k].length > maxN) { maxN = byModule[k].length; maxMod = k; } }
    if (!maxMod) return '本周暂无错题';
    return '本周在 <b>' + U.catInfo(maxMod).label + '</b> 板块错题较多（' + maxN + '题），为薄弱环节，建议重点回顾。';
  }
