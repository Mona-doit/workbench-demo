...
(function (global) {
  ...
  // 异形词/常用形归一：把历史记忆记录里的旧词条 key 迁移到新词条名，
  // 避免改名后丢失「已记忆」状态与艾宾浩斯复习进度（截至 2026-09-16：独具只眼 → 独具慧眼）
  const IDIOM_KEY_ALIAS = { 'im:独具只眼': 'im:独具慧眼' };
  function _migrateIdiomKeys() {
    let changed = false;
    try {
      const MQ = data.memoryQuiz;
      if (MQ && MQ.days && typeof MQ.days === 'object') {
        Object.keys(MQ.days).forEach(dt => {
          const arr = MQ.days[dt];
          if (!Array.isArray(arr)) return;
          arr.forEach(r => { if (r && IDIOM_KEY_ALIAS[r.key]) { r.key = IDIOM_KEY_ALIAS[r.key]; changed = true; } });
        });
      }
      const R = data.reviews;
      if (R && Array.isArray(R.memoryReviews)) {
        R.memoryReviews.forEach(r => { if (r && IDIOM_KEY_ALIAS[r.memoryKey]) { r.memoryKey = IDIOM_KEY_ALIAS[r.memoryKey]; changed = true; } });
      }
    } catch (e) {}
    if (changed) { try { write('memoryQuiz', data.memoryQuiz); write('reviews', data.reviews); } catch (e) {} }
    return changed;
  }
  ...
  data = _readAll();
  // 迁移历史「已记忆」记录 key（异形词改名后不丢记录/复习进度）
  try { _migrateIdiomKeys(); } catch (e) {}
  ...
  // 常识口诀已学统计：口诀卡「已背」(reviews.memoryReviews) 与记忆小测验作答(memoryQuiz.days) 两条路径合并去重
  function koujueMasteryStats() {
    const set = {};
    const reviews = (data.reviews && data.reviews.memoryReviews) || [];
    reviews.forEach(r => { if (r && r.memoryKey && r.memoryKey.indexOf('kj:') === 0) set[r.memoryKey] = 1; });
    const days = (data.memoryQuiz && data.memoryQuiz.days) || {};
    Object.keys(days).forEach(d => (days[d] || []).forEach(r => {
      if (r && r.key && r.key.indexOf('kj:') === 0 && r.result === 'known') set[r.key] = 1;
    }));
    return { learned: Object.keys(set).length };
  }
  // 政治理论核心考点已学统计：经记忆小测验作答(memoryQuiz.days, key=kd:title) 标记 known
  function kaodianMasteryStats() {
    const set = {};
    const days = (data.memoryQuiz && data.memoryQuiz.days) || {};
    Object.keys(days).forEach(d => (days[d] || []).forEach(r => {
      if (r && r.key && r.key.indexOf('kd:') === 0 && r.result === 'known') set[r.key] = 1;
    }));
    return { learned: Object.keys(set).length };
  }
  ...
  global.Store = {
    ...
    koujueMasteryStats, kaodianMasteryStats,
