...
(function (global) {
  ...
  function viewStats() {
    ...
    for (const key in mods) {
      ...
      let coverHTML = '';
      if (total > 0) {
        // 题库覆盖 = 已训练题数 / 题库总量（已训练集跨天去重，全部训练完即 100%）
        const done = Math.min((typeof U.dqTrainedCount === 'function') ? U.dqTrainedCount(key) : cnt, total);
  ...
  function bindDailyQuiz(module, pool, opts) {
    ...
    const trainedArr = (typeof U.dqTrainedLoad === 'function') ? U.dqTrainedLoad(module) : [];
    const trainedSet = {};
