  ...
  function bindPolitics() {
    ...
    document.querySelectorAll('[data-kd-title]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        // 联动：今日 5 个核心考点全部「✓ 已学」后，自动勾选今日待办「熟记5个核心考点」
        try {
          const allBtns = document.querySelectorAll('[data-kd-title]');
          const total = allBtns.length;
          const done = document.querySelectorAll('[data-kd-title].on').length;
          if (total > 0 && done >= total) {
            const entries = (typeof U.getModuleTodoEntries === 'function') ? U.getModuleTodoEntries('politics') : [];
            const cy = entries.find(function (en) { return en.title && en.title.indexOf('核心考点') !== -1; });
            if (cy) {
              const tKey = U.todoKey('politics', cy.index);
              const already = localStorage.getItem(Store.PREFIX + tKey) === '1';
              if (!already) {
                localStorage.setItem(Store.PREFIX + tKey, '1');
                if (S && typeof S.markModuleDone === 'function') S.markModuleDone('politics', cy.title);
                // 局部刷新「今日待办」卡片（政治理论），即时显示勾选
                if (typeof refreshModuleTodo === 'function') refreshModuleTodo('politics');
                // 同步刷新「今日计划」卡片，保持两处一致
                if (typeof bindModulePlanEvents === 'function') bindModulePlanEvents('politics');
                if (global.APP && typeof global.APP.updateBadges === 'function') global.APP.updateBadges();
                U.toast('🎯 今日「熟记5个核心考点」已完成！');
              }
            }
          }
        } catch (err) {}
  ...
  function bindLanguage() {
    ...
    document.querySelectorAll('[data-id-a]').forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        // 联动：今日成语辨析（1对）点「✓ 已掌握辨析」后，自动勾选今日待办「背诵成语辨析1对」
        try {
          const allBtns = document.querySelectorAll('[data-id-a]');
          const total = allBtns.length;
          const done = document.querySelectorAll('[data-id-a].on').length;
          if (total > 0 && done >= total) {
            const entries = (typeof U.getModuleTodoEntries === 'function') ? U.getModuleTodoEntries('language') : [];
            const cy = entries.find(function (en) { return en.title && en.title.indexOf('成语辨析') !== -1; });
            if (cy) {
              const tKey = U.todoKey('language', cy.index);
              const already = localStorage.getItem(Store.PREFIX + tKey) === '1';
              if (!already) {
                localStorage.setItem(Store.PREFIX + tKey, '1');
                if (S && typeof S.markModuleDone === 'function') S.markModuleDone('language', cy.title);
                // 局部刷新「今日待办」卡片（言语理解），即时显示勾选
                if (typeof refreshModuleTodo === 'function') refreshModuleTodo('language');
                // 同步刷新「今日计划」卡片，保持两处一致
                if (typeof bindModulePlanEvents === 'function') bindModulePlanEvents('language');
                if (global.APP && typeof global.APP.updateBadges === 'function') global.APP.updateBadges();
                U.toast('🎯 今日「背诵成语辨析1对」已完成！');
              }
            }
          }
        } catch (err) {}
