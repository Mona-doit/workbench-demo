...
(function (global) {
  ...
  function viewLanguage() {
    ...
    return moduleShell('language', '言语理解', '💬', [
      ...
      '<div class="card">' +
        '<div class="card-title"><span class="emoji">📖</span>今日成语辨析<span class="card-sub">（每日1对辨析 + 1组串讲 · 共 ' + C.IDIOMS.length + ' 对 / ' + (C.IDIOM_GROUPS ? C.IDIOM_GROUPS.length : 0) + ' 组）</span></div>' +
              ...
              '<div class="grp-items">' +
                grp.items.map(function (it) {
                  return '<div class="grp-item"><span class="grp-w">' + U.esc(it.word) + '</span>' +
                    (it.pinyin ? '<span class="grp-py">' + U.esc(it.pinyin) + '</span>' : '') +
