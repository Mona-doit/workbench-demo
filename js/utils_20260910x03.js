...
(function (global) {
  ...
  // ===== 已训练题集（跨天持久化）=====
  // 用途：某模块题库被"每日真题训练"全部作答过后，暂停该模块每日训练，
  //       直到用户扩充题库（total 增大）后再自动恢复；已训练的题不再出现在每日训练中。
  // 指纹与"每日去重计数"(dq_counted) 保持一致：题干去空白后前 60 字。
  // 这样首次部署时可从历史的 dq_counted_<module>_<date> 直接迁移，避免已刷过的题被重置。
  function dqFp(q) {
    return String(q == null ? '' : q).replace(/\s+/g, '').slice(0, 60);
  }
  function dqTrainedKey(module) { return 'dq_trained_' + module; }
  // 从历史的每日去重计数记录聚合出"已训练题集"（用于旧数据迁移，仅首次触发）
  function dqTrainedSeed(module) {
    const set = {};
    try {
      for (let i = 0; i < localStorage.length; i++) {
        const k = localStorage.key(i);
        if (!k || k.indexOf(Store.PREFIX + 'dq_counted_' + module + '_') !== 0) continue;
        try {
          const obj = JSON.parse(localStorage.getItem(k) || 'null');
          if (obj && typeof obj === 'object') Object.keys(obj).forEach(function (f) { if (f) set[f] = 1; });
        } catch (e) {}
      }
    } catch (e) {}
    return Object.keys(set);
  }
  function dqTrainedLoad(module) {
    try {
      // 首次访问且尚未初始化 → 用历史 dq_counted 播种（迁移旧进度），之后以该集合为准
      if (localStorage.getItem(Store.PREFIX + dqTrainedKey(module)) === null) {
        const seeded = dqTrainedSeed(module);
        try { localStorage.setItem(Store.PREFIX + dqTrainedKey(module), JSON.stringify(seeded)); } catch (e) {}
        return seeded;
      }
      const raw = localStorage.getItem(Store.PREFIX + dqTrainedKey(module));
      const a = raw ? JSON.parse(raw) : [];
      return Array.isArray(a) ? a : [];
    } catch (e) { return []; }
  }
  function dqTrainedMark(module, fps) {
    try {
      if (!fps || !fps.length) return;
      const set = {};
      dqTrainedLoad(module).forEach(function (f) { if (f) set[f] = 1; });
      fps.forEach(function (f) { if (f) set[f] = 1; });
      localStorage.setItem(Store.PREFIX + dqTrainedKey(module), JSON.stringify(Object.keys(set)));
    } catch (e) {}
  }
  function dqTrainedCount(module) {
    try { return dqTrainedLoad(module).length; } catch (e) { return 0; }
  }
  ...
  function flushAllDqStats() {
    try {
      ...
      modules.forEach(function (mod) {
        ...
        /* 2) 按「题目指纹」去重计数
              同一道题当天只计一次（重做不重复累加）；中途切走/关页面回来可正常补录。
              指纹由答题页写入 records[i].fp；旧数据无 fp 时按索引兜底，等价于旧的按数量逻辑。 */
        const ckey = P + 'dq_counted_' + mod + '_' + dateStr;
        let counted = null;
        try { counted = JSON.parse(localStorage.getItem(ckey) || 'null'); } catch (e) {}
        if (!counted || typeof counted !== 'object') {
          counted = {};
          let oldN = 0;
          try {
            oldN = parseInt(localStorage.getItem(P + 'dq_logged_' + mod + '_' + dateStr) || '0', 10) || 0;
          } catch (e) {}
          for (let i = 0; i < Math.min(oldN, records.length); i++) {
            counted[(records[i] && records[i].fp) || ('idx:' + i)] = 1;
          }
        }
        // 3) 统计本批新增中答对数
        let added = 0, newCorrect = 0;
        for (let i = 0; i < records.length; i++) {
          const r = records[i];
          if (!r) continue;
          const f = (r && r.fp) || ('idx:' + i);
          if (counted[f]) continue;
          counted[f] = 1;
          added++;
          if (r.correct) newCorrect++;
        }
        if (added <= 0) return;
        ...
        // 4) 写回指纹集合（并兼容维护旧的"已答数"标记）
        try { localStorage.setItem(ckey, JSON.stringify(counted)); } catch (e) {}
        try {
  ...
  // 模块每日真题训练是否已暂停（已训练题数 >= 题库总量）
  function dqPaused(cat) {
    try {
      const total = (typeof getModuleQuizTotal === 'function') ? getModuleQuizTotal(cat) : 0;
      if (!total) return false;
      const done = (typeof dqTrainedCount === 'function') ? dqTrainedCount(cat) : 0;
      return done >= total;
  ...
  function pendingTodos() {
    ...
    for (const cat in DEFAULT_MODULE_TODOS) {
      ...
      entries.forEach(e => {
        // 模块每日真题已暂停：该模块的「完成今日XX真题」待办视为已完成，不出现在今日待办
        if (dqPaused(cat) && /真题/.test(e.title)) return;
  ...
  global.Utils = {
    ...
    dqFp, dqTrainedLoad, dqTrainedMark, dqTrainedCount, dqTrainedKey, dqTrainedSeed, dqPaused,
